...
class EvidenceToolsTest(unittest.TestCase):
    def test_current_migration_chain_is_exact(self) -> None:
        ...
        self.assertEqual(
            "sha256:7d19f7d01fa2f47734d08b5159d1a87b30c55c8a54b798f15cc754b5c68bb74a",
            chain,
        )
        self.assertEqual("0029_postgresql_state_hardening.sql", items[-1]["name"])
    ...
    def test_command_sidecar_rejects_changed_log(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            summary.verify_command_sidecar(
                "unit", sidecar, root, schema, "db-test-command-001", source_digest, status_digest,
            )
            log.write_text("changed\n", encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "log digest mismatch"):
                summary.verify_command_sidecar(
                    "unit", sidecar, root, schema, "db-test-command-001",
                    source_digest, status_digest,
                )
    ...
    def test_supply_raw_closure_rejects_changed_scan(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for key, name in names.items():
                value = {"components": [{"name": "component"}]} if key.endswith("sbom") else {"Results": []}
                if key == "trivy_db_metadata":
                    value = {"UpdatedAt": "2026-08-20T00:00:00Z"}
                (root / name).write_text(json.dumps(value), encoding="utf-8")
            supply = {
                "digests": {key: summary.digest(root / name) for key, name in names.items()},
                "policy": {
            ...
            (root / "db.trivy.json").write_text(
                json.dumps({"Results": [{"Vulnerabilities": [{"Severity": "CRITICAL"}]}]}),
                encoding="utf-8",
            )
    ...
    def test_formal_soak_conversion_matches_shared_schema(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            subprocess.run([
                "python3", str(script_dir / "build-soak-evidence.py"),
                "--raw", str(raw), "--resources", str(resources),
                "--profile", str(profile), "--run-id", "db-test-soak-001",
                "--output", str(output),
            ], check=True)
            subprocess.run([
                "python3", str(script_dir / "validate-json.py"),
                "--schema", str(repo / "contracts/evidence/soak/v1/schema.json"),
                "--document", str(output),
