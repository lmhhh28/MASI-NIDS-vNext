...
EXPECTED_REQUIREMENTS = {
    "ARCH-003", "ARCH-REUSE-001", "CONTRACT-001", "CONTRACT-PROFILE-001",
    "DB-001", "DB-002", "DB-003", "DB-004", "DB-005", "DB-006", "DB-007",
    "DB-FW-001", "DB-GOV-001", "DB-MODEL-001", "DB-PLUGIN-001",
    "DB-PLUGIN-STAT-001", "DB-REDIS-001", "DB-RULE-001", "DB-TARGET-FLEET-001",
    "DEC-003", "DEC-039", "DEC-042", "DEC-044", "FUNC-FW-001", "FUNC-RULE-001",
    "MIG-001", "MIG-003", "MIG-005", "MIG-INF-001", "MIG-P4-FW-001",
    "MIG-PLUGIN-001", "MIG-TARGET-FLEET-001", "MIG-TEL-INF-001", "MOD-DB-001",
    "OBS-001", "OBS-002", "OBS-003", "PERF-001", "PERF-002", "PERF-P4-FW-001",
    "PERF-PLUGIN-STAT-001", "PERF-RULE-001", "PERF-TARGET-FLEET-001", "REL-001",
    "REL-002", "REL-003", "SEC-001", "SEC-003", "SEC-005", "SEC-SUPPLY-001",
    "TEST-002", "TEST-003", "TEST-007", "TEST-008", "TEST-009", "TEST-010",
    "TEST-GATE-001", "TEST-INF-001", "TEST-P4-FW-001", "TEST-PLUGIN-STAT-001",
    "TEST-REAL-E2E-001", "TEST-REUSE-001", "TEST-RULE-001", "TEST-TARGET-FLEET-001",
}
...
def main() -> int:
    ...
    if set(manifest) != {
        "schema_version", "module_id", "owner_role", "design_anchor", "commands",
        "execution_bindings", "public_contracts", "requirements", "conditional_applicability",
    }:
    ...
    if manifest["module_id"] != "MOD-DB-001" or manifest["owner_role"] != "PostgreSQL State module owner":
    ...
    for binding in bindings:
        if not isinstance(binding, dict) or set(binding) != {"command_id", "required", "targets"}:
            raise ValueError("execution binding fields mismatch")
        command_id = binding["command_id"]
        if not isinstance(command_id, str) or not COMMAND.fullmatch(command_id) or command_id in command_ids:
        ...
        if not isinstance(targets, list) or not 1 <= len(targets) <= 16 or any(
            not isinstance(target, str) or not 1 <= len(target) <= 128 for target in targets
        ):
            raise ValueError(f"invalid targets for {command_id}")
        command_ids.add(command_id)

    requirements = manifest["requirements"]
    if not isinstance(requirements, list) or len(requirements) != len(EXPECTED_REQUIREMENTS):
        raise ValueError("State requirement count mismatch")
    observed: set[str] = set()
    for requirement in requirements:
        if not isinstance(requirement, dict) or set(requirement) != {
            "requirement_id", "scenario_ids", "command_ids", "qualification_limit",
        }:
            raise ValueError("requirement mapping fields mismatch")
        requirement_id = requirement["requirement_id"]
        if not isinstance(requirement_id, str) or not ID.fullmatch(requirement_id) or requirement_id in observed:
        ...
        if not isinstance(scenarios, list) or not 1 <= len(scenarios) <= 16 or any(
            not isinstance(scenario, str) or not ID.fullmatch(scenario) for scenario in scenarios
        ):
            raise ValueError(f"invalid scenarios for {requirement_id}")
        if not isinstance(commands, list) or not 1 <= len(commands) <= 32 or len(commands) != len(set(commands)):
            raise ValueError(f"invalid commands for {requirement_id}")
        missing_commands = set(commands) - command_ids
        if missing_commands:
            raise ValueError(f"unknown commands for {requirement_id}: {sorted(missing_commands)}")
    ...
    if not isinstance(contracts, list) or not 1 <= len(contracts) <= 64 or len(contracts) != len(set(contracts)):
    ...
    observed_conditionals = {
        ...
        for item in conditionals if isinstance(item, dict)
    }
    if len(conditionals) != 4 or observed_conditionals != EXPECTED_CONDITIONALS or any(
        item.get("applicability") != "NOT_APPLICABLE" or item.get("result") != "NOT_RUN"
