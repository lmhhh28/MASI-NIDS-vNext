...
def validate_document(document: Path, schema_path: Path) -> None:
    ...
    errors = sorted(
        Draft202012Validator(schema, format_checker=FormatChecker()).iter_errors(read(document)),
        key=lambda error: list(error.absolute_path),
    )
    if errors:
        first = errors[0]
        location = "/".join(str(part) for part in first.absolute_path) or "$"
        raise ValueError(f"schema validation failed {document} at {location}: {first.message}")
...
def source_migration_chain(directory: Path) -> tuple[list[dict[str, Any]], str]:
    require(not directory.is_symlink() and directory.is_dir(), "unsafe migration directory")
    paths = sorted(directory.glob("*.sql"), key=lambda path: path.name)
    require(len(paths) == 29, f"expected 29 migrations, got {len(paths)}")
    ...
    for version, path in enumerate(paths, start=1):
        ...
        require(
            not path.is_symlink() and path.is_file() and 0 < path.stat().st_size <= 4_194_304,
...
def verify_migration_manifest(
    ...
    require(
        set(document) == {"schema_version", "chain_digest", "migration_count", "migrations"},
        "migration-chain document shape mismatch",
    )
    require(document["schema_version"] == "migration-chain-manifest/v1", "migration-chain schema mismatch")
    require(document["migration_count"] == 29, "migration-chain count mismatch")
    require(document["chain_digest"] == expected_digest, "migration-chain digest mismatch")
    require(document["migrations"] == expected_items, "migration-chain source closure mismatch")


def verify_migration_run(
    document: dict[str, Any], expected_names: list[str], expected_digest: str, fresh: bool
) -> None:
    require(
        set(document)
        == {
    ...
    require(document["database"] == "masi_state_module_test", "migration database mismatch")
    require(180000 <= document["server_version_num"] < 190000, "migration did not use PostgreSQL 18")
    require(document["tls"] is True, "migration TLS readback missing")
    require(document["schema_version"] == "21", "migration schema version mismatch")
    require(document["chain_digest"] == expected_digest, "migration result chain mismatch")
    require(document["history_table"] == "masi_migration_history", "migration history table mismatch")
    ...
    require(document["already_applied"] == expected_existing, "migration existing set mismatch")
    started, completed = parse_time(document["started_at"]), parse_time(document["completed_at"])
    require(completed >= started, "migration timestamps are reversed")
    require(type(document["elapsed_ms"]) is int and 0 <= document["elapsed_ms"] <= 600_000,
            "migration elapsed time outside bound")


def verify_catalog(document: dict[str, Any], expected_digest: str) -> None:
    expected_tables = {
        "database_recovery_events", "effect_attempts", "effect_decisions", "effect_intents",
        "effect_proposals", "event_identities", "events", "firewall_bindings",
        "firewall_revisions", "fleet_child_intents", "fleet_operations", "incidents",
        "masi_schema_meta", "model_control_incarnations", "model_control_state",
        "model_revisions", "plugin_bindings", "plugin_manifests", "plugin_qualifications",
        "plugin_statistic_artifacts", "plugin_statistic_runs", "plugin_statistic_schedules",
        "plugin_statistics_current", "plugin_statistics_definitions", "plugin_statistics_history",
        "pool_generations", "retention_holds", "rule_observation_epochs", "rule_observations",
        "rule_rollups_1h", "rule_rollups_5m", "shard_bindings", "target_assignments",
        "target_control_incarnations", "target_control_state", "targets",
    }
    expected_roles = {
        "masi_analysis_private", "masi_backup", "masi_control_app", "masi_control_maintenance",
        "masi_control_readonly", "masi_monitoring",
    }
    require(
        set(document)
        == {
            "database", "server_version_num", "tls", "in_recovery", "timeline_id",
            "schema_version", "schema_chain_digest", "migration_rows", "required_tables",
            "required_roles", "settings", "partitioned_parents", "invalid_constraints",
            "writer_gates",
        },
        "catalog readback shape mismatch",
    )
    require(document["database"] == "masi_state_module_test", "catalog database mismatch")
    require(180000 <= document["server_version_num"] < 190000, "catalog did not use PostgreSQL 18")
    require(document["tls"] is True and document["in_recovery"] is False, "catalog identity mismatch")
    require(type(document["timeline_id"]) is int and document["timeline_id"] >= 1,
            "catalog timeline mismatch")
    require(document["schema_version"] == "21", "catalog schema version mismatch")
    require(document["schema_chain_digest"] == expected_digest, "catalog chain mismatch")
    require(document["migration_rows"] == 29, "catalog migration row count mismatch")
    require(document["required_tables"] == {name: True for name in expected_tables},
            "catalog required-table closure mismatch")
    require(document["required_roles"] == {name: True for name in expected_roles},
    ...
    require(
        set(partitions) == {"events", "rule_rollups_5m", "rule_rollups_1h", "plugin_statistics_history"}
        and all(type(count) is int and count >= 2 for count in partitions.values()),
        "catalog partition coverage mismatch",
    )
    require(document["invalid_constraints"] == [], "catalog contains unvalidated constraints")
    require(document["writer_gates"] == {"model": False, "target": False},
            "catalog writer gates are open")


def verify_roles(document: dict[str, Any]) -> None:
    no_login = {
        "login": False, "superuser": False, "create_db": False, "create_role": False,
        "replication": False, "inherit": True, "connection_limit": -1,
    }
    expected = {
        ...
        "masi_bootstrap": {
            "login": False, "superuser": True, "create_db": True, "create_role": True,
            "replication": True, "inherit": True, "connection_limit": -1,
        },
        "masi_control_app": no_login,
        "masi_control_maintenance": no_login,
        "masi_control_readonly": no_login,
        "masi_migration_login": {**no_login, "login": True, "inherit": False, "connection_limit": 4},
        "masi_monitoring": no_login,
        "masi_monitoring_login": {**no_login, "login": True, "connection_limit": 4},
        "masi_replication_login": {
            **no_login, "login": True, "replication": True, "inherit": False,
            "connection_limit": 4,
        },
    }
    require(
        set(document)
        == {
            "database", "tls", "roles", "app_control_member", "app_backup_member",
            "monitoring_member", "migration_private_member", "migration_monitor_member",
            "public_connect", "public_temporary",
        },
        "runtime-role readback shape mismatch",
    )
    require(document["database"] == "masi_state_module_test" and document["tls"] is True,
            "runtime-role identity mismatch")
    require(document["roles"] == expected, "runtime-role exact matrix mismatch")
    require(document["app_control_member"] is True, "application control membership missing")
    for key in ("app_backup_member", "migration_private_member", "migration_monitor_member",
                "public_connect", "public_temporary"):
...
def verify_pool(document: dict[str, Any]) -> None:
    require(
        set(document)
        == {"tls", "version", "settings", "configured_database", "database_host", "database_port", "pool_rows"},
        "pool readback shape mismatch",
    )
    require(document["tls"] is True and document["version"] == "PgBouncer 1.25.2",
            "pool identity mismatch")
    require(
        document["settings"]
        == {
            "application_name_add_host": "1", "auth_type": "scram-sha-256",
            "client_tls_sslmode": "verify-full", "default_pool_size": "24",
            "max_client_conn": "256", "max_db_connections": "32",
            "max_packet_size": "16777216", "max_user_connections": "32",
            "pool_mode": "transaction", "query_timeout": "35", "query_wait_timeout": "10",
            "server_check_query": "SELECT 1", "server_reset_query": "DISCARD ALL",
            "server_tls_sslmode": "verify-full",
        },
        "pool settings mismatch",
    )
    require(document["configured_database"] is True and document["database_host"] == "postgres"
            and document["database_port"] == 5432, "pool database route mismatch")
    require(type(document["pool_rows"]) is int and 1 <= document["pool_rows"] <= 64,
...
def verify_application(document: dict[str, Any]) -> None:
    require(
        set(document)
        == {
            "database", "user", "server_version_num", "tls", "backend_pid",
            "can_insert_retention_hold", "can_delete_retention_hold",
            "can_use_analysis_private", "is_backup_member", "rolled_back_probe_write",
            "plaintext_fallback_rejected",
        },
        "application readback shape mismatch",
    )
    require(document["database"] == "masi_state_module_test" and document["user"] == "masi_app_login",
            "application identity mismatch")
    require(180000 <= document["server_version_num"] < 190000 and document["tls"] is True,
            "application PostgreSQL/TLS mismatch")
    require(type(document["backend_pid"]) is int and document["backend_pid"] > 0,
            "application backend PID invalid")
    require(document["can_insert_retention_hold"] is True, "application insert privilege missing")
    for key in ("can_delete_retention_hold", "can_use_analysis_private", "is_backup_member"):
        require(document[key] is False, f"application has forbidden capability: {key}")
    require(document["rolled_back_probe_write"] is True, "application probe was not rolled back")
    require(document["plaintext_fallback_rejected"] is True, "plaintext fallback was not rejected")
...
def verify_command_sidecar(
    ...
    require(sidecar["run_id"] == run_id and sidecar["command_id"] == command_id,
            f"command identity mismatch: {command_id}")
    require(sidecar["working_directory"] == "db", f"command working directory mismatch: {command_id}")
    require(sidecar["source_tree_digest"] == source_tree_digest,
            f"command source digest mismatch: {command_id}")
    require(sidecar["working_tree_status_digest"] == status_digest,
            f"command status digest mismatch: {command_id}")
    require(sidecar["result"] == "PASS" and sidecar["qualification"] == "QUALIFIED"
            and sidecar["exit_code"] == 0 and sidecar.get("stable_reason") is None,
            f"command did not pass: {command_id}")
    started, finished = parse_time(sidecar["started_at"]), parse_time(sidecar["finished_at"])
    require(finished >= started, f"command timestamps are reversed: {command_id}")
    expected_log = f"{command_id}.log"
    require(sidecar["log"]["path"] == expected_log, f"command log path mismatch: {command_id}")
    log = run_directory / expected_log
    require(sidecar["log"]["sha256"] == digest(log), f"command log digest mismatch: {command_id}")
    require(sidecar["log"]["bytes"] == log.stat().st_size, f"command log size mismatch: {command_id}")
    return sidecar


def verify_traceability(
    document: dict[str, Any], manifest: dict[str, Any], manifest_path: Path,
    run_directory: Path, sidecars: dict[str, dict[str, Any]],
) -> None:
    requirement_ids = [item["requirement_id"] for item in manifest["requirements"]]
    require(document["schema_version"] == "postgresql-state-requirement-traceability/v1",
            "traceability schema identity mismatch")
    require(document["module_id"] == "MOD-DB-001", "traceability module mismatch")
    require(document["manifest"] == "db/requirements-traceability.json",
            "traceability manifest path mismatch")
    require(document["manifest_digest"] == digest(manifest_path), "traceability manifest digest mismatch")
    require(document["evidence_root"] == str(run_directory), "traceability evidence root mismatch")
    require(document["result"] == "PASS" and document["qualification"] == "QUALIFIED"
            and document["valid"] is True and document["failures"] == [],
            "traceability is not a qualified PASS")
    require(document["requirement_ids"] == requirement_ids, "traceability requirement set mismatch")
    require(document["conditional_applicability"] == manifest["conditional_applicability"],
            "traceability conditional applicability mismatch")
    requirements = document["requirements"]
    require(len(requirements) == len(manifest["requirements"]), "traceability requirement count mismatch")
    ...
    for actual, expected in zip(requirements, manifest["requirements"], strict=True):
        require(actual["requirement_id"] == expected["requirement_id"], "traceability requirement order mismatch")
        require(actual["scenario_ids"] == expected["scenario_ids"], "traceability scenario mismatch")
        require(actual["qualification_limit"] == expected["qualification_limit"],
                "traceability qualification limit mismatch")
        require(actual["evidence_result"] == "PASS" and actual["qualification"] == "QUALIFIED",
                "traceability requirement did not pass")
        require([item["command_id"] for item in actual["test_targets"]] == expected["command_ids"],
                "traceability target command mismatch")
        require([item["target"] for item in actual["test_targets"]]
                == [",".join(bindings[command_id]["targets"]) for command_id in expected["command_ids"]],
                "traceability target name mismatch")
        require([item["path"] for item in actual["evidence"]]
                == [f"{command_id}.command.json" for command_id in expected["command_ids"]],
                "traceability evidence path mismatch")
        require(all(item["result"] == "PASS" and item["qualification"] == "QUALIFIED"
                    for item in actual["test_targets"] + actual["evidence"]),
                "traceability contains a non-PASS binding")
    expected_artifacts = {
        "db/requirements-traceability.json": (digest(manifest_path), manifest_path.stat().st_size),
        **{
    ...
    require(observed_artifacts == expected_artifacts, "traceability artifact closure mismatch")
...
def main() -> int:
    ...
    parser.add_argument("--working-tree-dirty", required=True, choices=("true", "false"))
    ...
    require(args.run_directory.is_absolute() and not args.run_directory.is_symlink()
            and args.run_directory.is_dir(), "unsafe run directory")
    manifest = read(args.manifest)
    require(manifest.get("module_id") == "MOD-DB-001", "requirement manifest module mismatch")
    require(len(manifest.get("requirements", [])) == 64, "requirement manifest must contain 64 items")
    require(all(binding.get("required") is True for binding in manifest.get("execution_bindings", [])),
    ...
    schemas = {
        ...
        "capacity": repo_root / "contracts/evidence/postgresql-state-capacity/v1/schema.json",
        "recovery": repo_root / "contracts/evidence/postgresql-state-recovery/v1/schema.json",
        ...
        "supply": repo_root / "contracts/evidence/postgresql-state-supply/v1/schema.json",
        "module": repo_root / "contracts/evidence/v1/postgresql-state-module-schema.json",
    }
    ...
    require(findings["module_id"] == "MOD-DB-001" and findings["decision_id"] == "DEC-044",
    ...
    required_artifacts = [
        formal, formal_raw, formal_resources, capacity, capacity_raw, recovery, backup_manifest,
        pitr_before, pitr_rotation, pitr_after, replica_before, replica_after, oci, supply,
        traceability, migration_chain, fresh, repeat, disk_fault, catalog, roles, pool, application,
    ]
    ...
    expected_migrations, expected_chain = source_migration_chain(args.manifest.parent / "migrations")
    migration_chain_document = read(migration_chain)
    verify_migration_manifest(migration_chain_document, expected_migrations, expected_chain)
    ...
    verify_migration_run(disk_fault_document, expected_names, expected_chain, fresh=True)
    ...
    require(formal_document["run_id"] == args.run_id and formal_document["module"] == "postgresql-state",
            "formal soak identity mismatch")
    require(formal_document["level"] == "MODULE" and formal_document["applicability"] == "APPLICABLE"
            and formal_document["result"] == "PASS" and formal_document["qualification"] == "QUALIFIED",
            "formal soak is not a qualified module PASS")
    require(formal_document["profile_digest"]
    ...
    require(
        ...
        == {
            ...
            "resource-samples.json": (digest(formal_resources), formal_resources.stat().st_size),
        },
    ...
    require(capacity_document["run_id"] == args.run_id and capacity_document["result"] == "PASS"
            and capacity_document["qualification"] == "QUALIFIED", "capacity evidence mismatch")
    capacity_raw_document = read(capacity_raw)
    require(capacity_raw_document.get("schema_version") == "postgresql-state-workload/v1"
            and capacity_raw_document.get("mode") == "capacity"
            and capacity_raw_document.get("result") == "PASS", "capacity raw identity mismatch")
    require(capacity_raw_document.get("identity") == capacity_document["identity"]
    ...
    require(recovery_document["run_id"] == args.run_id and recovery_document["result"] == "PASS"
            and recovery_document["qualification"] == "QUALIFIED", "recovery evidence mismatch")
    require(
    ...
    backup_match = re.fullmatch(r"([0-9a-f]{64})  /base/18/docker/backup_manifest\n", backup_text)
    require(backup_match is not None, "backup manifest checksum evidence shape mismatch")
    require("sha256:" + backup_match.group(1) == recovery_document["pitr"]["backup_manifest_digest"],
    ...
    require(oci_document["image_ids"] == expected_image_ids, "OCI image identity mismatch")
    require(oci_document["migration_count"] == 29
            and oci_document["migration_chain_digest"] == expected_chain, "OCI migration closure mismatch")
    ...
    require(supply_document["image_ids"] == expected_image_ids, "supply image identity mismatch")
    require(supply_document["images"] == oci_document["images"], "OCI/supply image tags differ")
    ...
    verify_traceability(traceability_document, manifest, args.manifest, args.run_directory, sidecars)
    ...
    complete = all_pass and formal_pass and capacity_pass and recovery_pass and not any(
        item["severity"] == "P0" for item in open_findings
    )
    artifacts = {
        "migration_chain": expected_chain,
        "migration_0029": digest(args.manifest.parent / "migrations/0029_postgresql_state_hardening.sql"),
        "requirements_baseline": digest(repo_root / "docs/masi-nids-vnext-system-requirements-2026-08-09.md"),
        ...
        "performance_profile": digest(repo_root / "contracts/profiles/v1/performance-environment-postgresql-state.json"),
        "reliability_profile": digest(repo_root / "contracts/profiles/v1/reliability-environment-postgresql-state-single-domain.json"),
    ...
    document = {
        ...
        "completion": {
            "operational_gates_pass": all_pass and formal_pass and capacity_pass and recovery_pass,
            "open_p0_zero": not any(item["severity"] == "P0" for item in open_findings),
            "real_runtime_started": executed.get("fresh-migration") == "PASS" and executed.get("pool-readback") == "PASS",
            "formal_soak_executed": formal_pass,
            "no_required_not_run": all(value != "NOT_RUN" for value in executed.values())
        },
        "blackbox_e2e": {"result": executed.get("blackbox", "NOT_RUN"), "evidence": "blackbox.log", "digest": digest(args.run_directory / "blackbox.log")},
        "recovery": {"result": "PASS" if recovery_pass else "FAIL", "evidence": "recovery/recovery-evidence.json", "digest": digest(recovery)},
        "performance": {"result": "PASS" if capacity_pass else "FAIL", "evidence": "capacity/capacity-evidence.json", "digest": digest(capacity)},
        "formal_soak": {"result": "PASS" if formal_pass else "FAIL", "evidence": "formal-soak/formal-soak-evidence.json", "digest": digest(formal)},
        "findings": {
