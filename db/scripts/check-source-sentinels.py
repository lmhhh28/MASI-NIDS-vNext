...
TOKEN = re.compile(
    "|".join(("TO" + "DO", "FIX" + "ME", "place" + "holder", "st" + "ub", "hardcoded success", "temporary compatibility")),
    re.IGNORECASE,
)
...
def main() -> int:
    ...
    for root in roots:
        for path in sorted(root.rglob("*")):
            ...
            if (
                ...
                or any(part in {"evidence", "out", "__pycache__"} for part in relative_parts)
                or path.is_symlink()
                or not path.is_file()
    ...
    if unexpected:
        raise ValueError("live implementation sentinel(s):\n" + "\n".join(unexpected[:64]))
    ...
    incident = (repo / "db/migrations/0014_event_outcome_incident_projection.sql").read_text(encoding="utf-8")
    if "SET checksum=$1,source_digest=$2" not in runner or '"migration-chain/"+cfg.ExpectedSchemaVersion' not in runner:
        raise ValueError("migration runner no longer publishes the exact final chain digest")
    ...
    if "CREATE TABLE IF NOT EXISTS incident_projection_events" not in incident:
        raise ValueError("incident projection did not supersede the immutable phase-2 bootstrap comment")
    print("postgresql-state-source-sentinels-ok live=0 immutable_audited=2")
    return 0
