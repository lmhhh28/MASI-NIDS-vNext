...
watch(
  () => session.data.value,
  (value) => {
    if (value !== undefined) invalidationStream.start(queryClient)
    else invalidationStream.stop()
  },
  { immediate: true },
)
</script>
...
.login-boundary__action { margin-top: var(--space-3); display: inline-flex; min-height: 2.75rem; align-items: center; padding: 0 var(--space-4); border-radius: var(--radius-sm); background: var(--action-primary); color: white; font-weight: 700; text-decoration: none; }
</style>
