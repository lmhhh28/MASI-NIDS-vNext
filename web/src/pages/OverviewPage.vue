...
const query = useQuery({
  queryKey: sessionBoundKey('dashboard'),
...
.target-strip__items { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); }.target-strip article { min-width: 0; display: grid; gap: var(--space-2); padding: var(--space-3); border-right: 1px solid var(--border-subtle); }.target-strip article:last-child { border: 0; }.target-strip article div { display: grid; }.target-strip article > small, .target-strip article > span:not(.status-mark), .target-strip article > time, .target-strip article > div > small, .target-strip article > div > span, .target-strip article > div > time { overflow: hidden; color: var(--text-muted); font-size: var(--text-xs); text-overflow: ellipsis; white-space: nowrap; }
