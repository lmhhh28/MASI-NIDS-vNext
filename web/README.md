...
```bash
MASI_WEB_FORMAL_SOAK=1 scripts/run-module-gates.sh
```

Evidence is published under `web/evidence/module-gates/runs/<run-id>/`; `gate-summary.json` and `latest.json` are immutable-run pointers. DEC-044 operational completion remains separate from protected-baseline, formal pairwise, system, and production qualification.
