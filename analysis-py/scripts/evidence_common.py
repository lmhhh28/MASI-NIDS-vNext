#!/usr/bin/env python3
"""Shared fail-closed file and source-snapshot helpers for Analysis evidence."""
...
def safe_run_file(run_dir: Path, relative: str) -> Path:
    ...
    if relative_path.is_absolute() or relative != relative_path.as_posix() or ".." in relative_path.parts:
...
def safe_summary_input(run_dir: Path, path: Path) -> Path:
    ...
    try:
        ...
        raise ValueError("module summary must be inside the immutable run directory") from exc
...
def safe_output(run_dir: Path, path: Path) -> Path:
    ...
    try:
        ...
        raise ValueError("module summary output must be inside the immutable run directory") from exc
    if relative == Path(".") or len(relative.parts) != 1 or ".." in relative.parts or "\\" in relative.as_posix():
...
def write_new_output(run_dir: Path, path: Path, payload: bytes) -> None:
    ...
    directory = os.open(run_dir, os.O_RDONLY | os.O_CLOEXEC | os.O_DIRECTORY | os.O_NOFOLLOW)
    try:
        descriptor = os.open(
            path.name,
...
def current_snapshot(repo: Path) -> tuple[str, str, bool, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="masi-analysis-source-digest.") as temporary:
