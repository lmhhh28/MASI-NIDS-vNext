...
“应产证据”列描述未来必须产生的证据类别，不表示当前已存在。截至 2026-08-20：P4/Switch 已取得 `result=PASS, qualification=QUALIFIED`；Rust Edge、Central Inference 已达到 operational Module Complete，但 qualification 仍为 `HOLD/NOT_QUALIFIED`。Go Control 不在本矩阵硬编码易失的完成结论或旧run ID，只以 [`control-go/evidence/module-gates/latest.json`](../../control-go/evidence/module-gates/latest.json) 指向的、与当前source tree/binary/OCI/formal-soak digest一致且`overall_module_complete=true`的证据为准。PostgreSQL、Plugin Host、Python Analysis、Web、Offline ML 尚未开始实现。各模块不得从其他模块继承完成或资格。
...
| PostgreSQL | PostgreSQL module owner | `MOD-DB-001`、`DB-005`、`DB-006`、`DB-MODEL-001`、`DB-RULE-001`、`DB-FW-001`、`DB-TARGET-FLEET-001`、`DB-PLUGIN-STAT-001` | [`postgresql-state-design.md`](../design/modules/postgresql-state-design.md) | [`TEST-003`/DB专项](../testing/module-e2e-acceptance-design.md#module-postgresql) | [P4–P7、P9、P12依赖](../integration/pairwise-and-system-integration-design.md#p4)；[W3–W10](../integration/pairwise-and-system-integration-design.md#system-waves) | migration/constraint/role、HA/PITR/restore/incarnation、RPO/RTO、query/retention/performance | `NOT_STARTED` — 仅 `db/migrations` 被 control-go 占用，未作为独立模块资格化 |
...
本矩阵已建立设计、验收和集成的稳定导航入口。截至 2026-08-20，契约/profile/golden 已冻结（Gate 0）；运行面四模块已实现并产生独立模块证据：P4/Switch 已 `PASS/QUALIFIED`（formal run `20260813T060546Z-runner-version-formal-006`，`remaining_holds=[]`，ISSUE-P4-SW-001 CLOSED），Rust Edge / Central Inference / Go Control 已 operational Module Complete（`overall_module_complete=true`、open P0=0），但 qualification 仍 `HOLD/NOT_QUALIFIED`——`DEC-001` 生产绝对门槛未冻结、受保护发布基线未形成、formal pairwise/system 未运行。PostgreSQL（仅 `db/migrations` 被 control-go 占用，未独立资格化）、Plugin Host、Python Analysis、Web（仅 `contracts/web/v1`+OpenAPI 冻结）、Offline ML 尚未开始实现。各 Owner 稳定身份须在模块实施启动前登记；表中 Owner 角色不能替代实际负责人。所有未来证据链接须在真实执行后以 immutable ID/digest 补充；当前不得将矩阵视为任何尚未实现模块或全局 system PASS。
