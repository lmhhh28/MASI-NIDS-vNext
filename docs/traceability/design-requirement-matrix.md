...
“应产证据”列描述未来必须产生的证据类别，不表示当前已存在。截至 2026-08-21：P4/Switch 已取得 `result=PASS, qualification=QUALIFIED`；Rust Edge、Central Inference、Go Control、PostgreSQL State、Plugin Runtime Host 已达到 operational Module Complete，但 qualification 仍为 `HOLD/NOT_QUALIFIED`。Go Control、PostgreSQL State与Plugin Runtime Host不在本矩阵用旧run覆盖当前源码，只分别以 [`control-go/evidence/module-gates/latest.json`](../../control-go/evidence/module-gates/latest.json)、[`db/evidence/module-gates/latest.json`](../../db/evidence/module-gates/latest.json)和[`plugin-host-rs/evidence/module-gates/latest.json`](../../plugin-host-rs/evidence/module-gates/latest.json) 指向的、与对应source tree/binary/OCI/formal-soak digest一致且`overall_module_complete=true`的证据为准。Python Analysis、Web、Offline ML尚未开始实现。各模块不得从其他模块继承完成或资格。
...
本矩阵已建立设计、验收和集成的稳定导航入口。截至 2026-08-21，契约/profile/golden 已冻结（Gate 0）；六个独立模块已实现并产生独立证据：P4/Switch 已 `PASS/QUALIFIED`（formal run `20260813T060546Z-runner-version-formal-006`，`remaining_holds=[]`，ISSUE-P4-SW-001 CLOSED），Rust Edge / Central Inference / Go Control / PostgreSQL State / Plugin Runtime Host 已 operational Module Complete（`overall_module_complete=true`、open P0=0），但 qualification 仍 `HOLD/NOT_QUALIFIED`——受保护发布基线和formal pairwise/system未取得；PostgreSQL另受single-domain/manual-promotion限制，Plugin Host另受single-domain Host、Go/DB pairwise和core相对退化门禁限制，均不能外推production-ha。Python Analysis、Web（仅`contracts/web/v1`+OpenAPI冻结）、Offline ML尚未开始实现。各Owner稳定身份须在模块实施启动前登记；表中Owner角色不能替代实际负责人。所有未来证据链接须在真实执行后以immutable ID/digest补充；当前不得将矩阵视为任何尚未实现模块或全局system PASS。
