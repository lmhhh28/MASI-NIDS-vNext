...
// ---------------------------------------------------------------------------
// SHA-256 through the production helper (this target links the Gateway library).
// ---------------------------------------------------------------------------
std::string sha256_hex(const std::string &data) {
  return masi::inf::sha256_hex(data.data(), data.size());
}
...
void test_inference_record_frozen_bytes() {
  ...
  for (const auto &s : rec_json["shape"])
  ...
  CHECK(digest == "sha256:e0eb77b45bc091fea08fd5b251841222a35c861a554337d6dfe815feda310bcc",
  ...
  CHECK(sha256_hex(again) == digest, "InferenceRecord serialization is not deterministic");
}
...
void test_inference_input_batch_frozen_bytes() {
  ...
  CHECK(sha256_hex(again) == digest, "InferenceInputBatch serialization is not deterministic");
...
void test_result_fence_dimensions() {
  ...
  for (size_t i = 0; i < 29; ++i) {
    ...
    CHECK(got == kExpected[i], "result_fence dimension " + std::to_string(i) + " mismatch: got '" +
...
void test_fallback_matrix() {
  ...
  CHECK(fb["old_model"].get<bool>() == false, "fallback.old_model must be false");
  ...
  CHECK(fb["json_or_http"].get<bool>() == false, "fallback.json_or_http must be false");
}
...
void test_golden_evidence_schema_versions() {
  ...
  static const Entry entries[] = {
      {"central-inference-blackbox-v1.json", "central-inference-module-e2e-evidence/v1"},
      {"central-inference-command-execution-v1.json", "edge-command-execution/v1"},
      {"central-inference-deep-check-v1.json", "central-inference-deep-check-evidence/v1"},
      {"central-inference-deep-not-run-v1.json", "central-inference-deep-check-evidence/v1"},
      {"central-inference-module-findings-v1.json", "module-findings/v1"},
      {"central-inference-module-v1.json", "central-inference-module-gate-summary/v1"},
      {"central-inference-numeric-v1.json", "central-inference-numeric-evidence/v1"},
      {"central-inference-oci-not-run-v1.json", "central-inference-oci-startup-evidence/v1"},
      {"central-inference-oci-startup-v1.json", "central-inference-oci-startup-evidence/v1"},
      {"central-inference-readback-v1.json", "central-inference-readback-evidence/v1"},
      {"central-inference-startup-v1.json", "central-inference-startup-evidence/v1"},
      {"central-inference-supply-failure-v1.json", "central-inference-supply-verification/v1"},
      {"central-inference-supply-not-run-v1.json", "central-inference-supply-verification/v1"},
      {"central-inference-supply-verification-v1.json", "central-inference-supply-verification/v1"},
      {"central-inference-traceability-v1.json", "central-inference-traceability-evidence/v1"},
  };
  for (const auto &e : entries) {
    auto j = load_golden_evidence(e.file);
    std::string sv = j["schema_version"].get<std::string>();
    CHECK(sv == e.expected_schema, std::string("golden evidence ") + e.file +
...
void test_golden_catalog() {
  ...
  for (const auto &v : vectors) {
    std::string path = masi::inf::test::contract_path("contracts/golden/inference/" +
...
void test_golden_adapter_cross_check() {
  const auto bundle = masi::inf::test::load_json(
      ...
      "/testkit/fixtures/repositories/masi-ids-window-v1-r3/bundle-manifest.json");
  ...
  p.ood_mode = bundle["output_adapter"]["ood_policy"]["mode"].get<std::string>();
  p.ood_below = bundle["output_adapter"]["ood_policy"]["threshold"].get<double>();
  ...
  for (const auto &name : vectors) {
    ...
    for (size_t i = 0; i < rows.size(); ++i) {
      ...
      for (const auto &v : rows[i]["raw_scores"])
        raw.push_back(v.get<float>());
      const auto adapted = masi::inf::apply_output_adapter(raw, p);
      CHECK(adapted.decision == rows[i]["decision"].get<std::string>(),
            name + " row " + std::to_string(i) + ": adapter decision '" + adapted.decision +
                "' != golden");
      ...
      if (rows[i].contains("out_of_distribution")) {
        CHECK(adapted.out_of_distribution == rows[i]["out_of_distribution"].get<bool>(),
              name + " row " + std::to_string(i) + ": OOD flag != golden");
      }
      ...
      for (size_t k = 0; k < adapted.scores.size(); ++k) {
        ...
        CHECK(std::fabs(got - want) <= 1e-6 + 1e-5 * std::fabs(want),
              name + " row " + std::to_string(i) + ": score " + std::to_string(k) + " != golden");
      }
      ...
      const std::string digest = masi::inf::sha256_hex(canonical.data(), canonical.size());
