...
void require_regular_no_symlink(const std::string &path) {
  ...
  if (lstat(path.c_str(), &st) != 0)
    fail_bad_pem(path, "lstat failed");
  if (S_ISLNK(st.st_mode))
    fail_bad_pem(path, "is symlink");
  if (!S_ISREG(st.st_mode))
    fail_bad_pem(path, "not regular file");
}
...
std::string read_pem_file(const std::string &path, bool is_private_key) {
  ...
  if (stat(path.c_str(), &st) != 0)
    fail_bad_pem(path, "stat failed");
  if (st.st_size > 1048576)
    fail_bad_pem(path, "exceeds 1MiB");
  if (is_private_key) {
    if ((st.st_mode & 077) != 0)
  ...
  if (!f)
    fail_bad_pem(path, "open failed");
  std::string content((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>());
  if (content.empty())
    fail_bad_pem(path, "empty");
...
grpc::SslServerCredentialsOptions build_server_mtls_options(const std::string &ca_path,
  ...
  opts.pem_key_cert_pairs.push_back(grpc::SslServerCredentialsOptions::PemKeyCertPair{
      read_pem_file(key_path, true), read_pem_file(cert_path, false)});
  return opts;
}
