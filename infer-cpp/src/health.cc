...
HealthMonitor::HealthMonitor(std::shared_ptr<CentralInferenceServiceImpl> svc,
  ...
  if (startup_.ready) state_.store(HealthState::kReady, std::memory_order_release);
}

void HealthMonitor::mark_ready() {
...
bool HealthMonitor::readiness_ok() const {
  if (state() != HealthState::kReady) return false;
  if (!svc_ || !svc_->accepting()) return false;
  if (!triton_ || !triton_->is_server_ready()) return false;
  if (!startup_.ready) return false;
  return true;
}
