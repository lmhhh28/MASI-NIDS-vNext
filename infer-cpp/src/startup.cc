...
[[noreturn]] void fail_stage(StageResult& s, const std::string& reason) {
  ...
  throw error::Exception(error::Code::kStartupFailed,
                         "stage " + s.stage + " failed: " + reason);
}
...
TritonModelConfigProjection parse_pinned_config_text(const std::string& text) {
  ...
  while (std::getline(in, line)) {
    ...
    if (t.rfind("input", 0) == 0 && t.find('[') != std::string::npos) { section = 1; brace_depth = 0; continue; }
    if (t.rfind("output", 0) == 0 && t.find('[') != std::string::npos) { section = 2; brace_depth = 0; continue; }
    if (t.rfind("dynamic_batching", 0) == 0) { section = 3; c.dynamic_batching = true; brace_depth = 0; continue; }
    if (t.rfind("instance_group", 0) == 0) { section = 4; brace_depth = 0; continue; }

    if (t == "{") { ++brace_depth; continue; }
    if (t == "}" || t == "},") {
    ...
    if (section == 0) {
      ...
      else if (key == "backend") c.backend = value;
      else if (key == "platform") c.platform = value;
      else if (key == "max_batch_size") c.max_batch_size = static_cast<int32_t>(std::strtol(value.c_str(), nullptr, 10));
    } else if (section == 1) {
      if (key == "name") c.input_name = value;
      else if (key == "data_type") c.input_datatype = value;
      else if (key == "dims") c.input_dims = parse_int_list(value);
    } else if (section == 2) {
      if (key == "name") c.output_name = value;
      else if (key == "data_type") c.output_datatype = value;
      else if (key == "dims") c.output_dims = parse_int_list(value);
    } else if (section == 3) {
      if (key == "preferred_batch_size") {
        for (int64_t v : parse_int_list(value)) c.preferred_batch_size.push_back(static_cast<int32_t>(v));
      ...
      if (key == "kind") pending_instance_kind = value;
      else if (key == "count") pending_instance_count = static_cast<int32_t>(std::strtol(value.c_str(), nullptr, 10));
    }
  }
  if (section == 4 && !pending_instance_kind.empty())
    c.instance_groups.emplace_back(pending_instance_kind, pending_instance_count);
...
std::string normalize_dtype(const std::string& s) {
  if (s.rfind("TYPE_", 0) == 0) return s.substr(5);
  return s;
}
...
void assert_triton_config_matches(const TritonModelConfigProjection& live,
  ...
  auto fail = [](const std::string& what) {
    throw error::Exception(error::Code::kIncompatibleContract,
                           "triton config mismatch: " + what);
  };

  // 1. live vs pinned on-disk config.pbtxt (proves Triton serves this closure).
  if (live.name != pinned.name) fail("name live=" + live.name + " pinned=" + pinned.name);
  ...
  if (live.instance_groups[0].second != env.instance_group.count)
    throw error::Exception(error::Code::kProviderPartitionDrift,
                           "instance_group count observed=" +
                               std::to_string(live.instance_groups[0].second) +
  ...
  if (env.runtime_profile_id == "model-runtime-central-cpu/v1") {
    ...
    throw error::Exception(error::Code::kRuntimeProfileUnsupported,
                           "runtime profile not qualified in this build: " + env.runtime_profile_id);
  }
}
...
StartupResult run_startup(const Config& cfg, TritonClient& triton, int64_t now_unix_ms) {
  ...
  // Stage 5: triton-none-load. Everything here is observed, never assumed.
  {
    ...
    try {
      ...
      const auto pinned = parse_pinned_config_text(
          read_closure_config_text(cfg.model_repository_path));
      ...
      proj << "server_name=" << r.observed.server.name << '\n'
           ...
           << r.observed.model.raw_metadata
           << r.observed.config.canonical_projection;
  ...
  // test harness, not in the production startup path.
  {
    ...
    try {
      ...
      auto first = triton.model_infer(r.triton_model_name, r.triton_model_version,
                                      r.observed.config.input_name, "UINT64",
                                      one, one_shape, cfg.request_deadline_ms);
      ...
      auto batched = triton.model_infer(r.triton_model_name, r.triton_model_version,
                                        r.observed.config.input_name, "UINT64",
                                        two, two_shape, cfg.request_deadline_ms);
